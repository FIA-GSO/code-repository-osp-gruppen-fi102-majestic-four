/*
  ...
  - Made the column `statusId` on table `Vortrag` required. This step will fail if there are existing NULL values in that column.
  - Made the column `statusId` on table `Stand` required. This step will fail if there are existing NULL values in that column.
...
CREATE TABLE "new_Vortrag" (
    ...
    CONSTRAINT "Vortrag_statusId_fkey" FOREIGN KEY ("statusId") REFERENCES "Status" ("id") ON DELETE RESTRICT ON UPDATE CASCADE
);
...
CREATE TABLE "new_Stand" (
    ...
    CONSTRAINT "Stand_statusId_fkey" FOREIGN KEY ("statusId") REFERENCES "Status" ("id") ON DELETE RESTRICT ON UPDATE CASCADE
);
